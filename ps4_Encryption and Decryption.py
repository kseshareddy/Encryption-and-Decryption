...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(26):
            if i + shift < 26 :
                self.d[l[i]] = l[i+shift]
            else:
                self.d[l[i]] = l[i+shift-26]
        for i in range(26):
            if i + shift < 26 :
                self.d[u[i]] = u[i+shift]
            else:
                self.d[u[i]] = u[i+shift-26]
        return self.d
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(26):
              hit = 0  
              self.cur = self.apply_shift(shift)
              for word in self.cur.split():
                  if is_word(self.get_valid_words(), word):
                      hit = hit + 1
        
              self.best[hit]= (shift,self.cur)
        return self.best[max(self.best.keys())]
